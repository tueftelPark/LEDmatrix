...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var AUTOSAVE_KEY = 'tueftelpark-led-autosave';

        function autoSave() {
            try {
                var data = {
                    patterns: framesToPatterns(),
                    canvas: converter.fixPattern($hexInput.value)
                };
        ...
        // ---------- Malen: Klick halten und über Felder ziehen ----------
        var isPainting = false;
        var paintMode = null; // 'draw' oder 'erase'

        function paintCell(item, mode) {
            if (mode === 'draw') {
        ...
        $leds.querySelectorAll('.item').forEach(function (item) {
            item.addEventListener('mousedown', function () {
                ...
                paintMode = wasActive ? 'erase' : 'draw';
                paintCell(this, paintMode);
                isPainting = true;
                ledsToHex();
            });

            item.addEventListener('mouseenter', function () {
                if (!isPainting || pipetteActive) return;
                paintCell(this, paintMode);
                ledsToHex();
            });
        });

        function itemFromTouch(touch) {
            var el = document.elementFromPoint(touch.clientX, touch.clientY);
            if (el && el.classList && el.classList.contains('item') && $leds.contains(el)) {
                return el;
            }
            return null;
        }

        $leds.addEventListener('touchstart', function (e) {
            var el = itemFromTouch(e.touches[0]);
            if (el) {
                e.preventDefault();
        ...
        $leds.addEventListener('touchmove', function (e) {
        ...
        // ---------- Drehen & Spiegeln ----------
        function transformMatrix(mapCell) {
            var hex = getInputHexValue();
            var grid = [];
            for (var r = 0; r < 8; r++) {
                grid.push([]);
                for (var c = 0; c < 8; c++) {
                    grid[r].push(hex.substr(r * 16 + c * 2, 2));
                }
            }
            var out = [];
            for (var r2 = 0; r2 < 8; r2++) {
                for (var c2 = 0; c2 < 8; c2++) {
                    out.push(mapCell(grid, r2, c2));
                }
            }
            $hexInput.value = patternToHexInputValue(out.join(''));
            hexInputToLeds();
        }

        document.getElementById('rotate-left-button').addEventListener('click', function () {
            transformMatrix(function (grid, r, c) {
                return grid[c][7 - r];
            });
        });
        ...
        document.getElementById('mirror-h-button').addEventListener('click', function () {
            transformMatrix(function (grid, r, c) {
                return grid[r][7 - c];
            });
        });

        document.getElementById('mirror-v-button').addEventListener('click', function () {
            transformMatrix(function (grid, r, c) {
                return grid[7 - r][c];
            });
        });
        ...
        function hasAutosaveContent(data) {
            if (!data) return false;
            if (data.patterns && data.patterns.length) return true;
            if (data.canvas && /[^f]/i.test(data.canvas)) return true;
            return false;
        }

        function restoreAutosave(data) {
            var patterns = (data.patterns || []).map(converter.fixPattern);
            $frames.innerHTML = '';
            for (var i = 0; i < patterns.length; i++) {
                $frames.appendChild(makeFrameElement(patterns[i]));
            }
            updateFrameCount(patterns.length);
            var canvas = (data.canvas && data.canvas.length === 128) ? data.canvas : 'ff'.repeat(64);
            $hexInput.value = patternToHexInputValue(canvas);
            printArduinoCode(patterns);
            hexInputToLeds();
            savedHashState = patterns.join('|');
            window.location.hash = savedHashState;
        ...
        function showAutosaveNotice() {
            ...
            $toast.innerHTML = '✓ Letzter Stand automatisch geladen <button id="discard-autosave-button" type="button">Neu beginnen</button>';
            document.body.appendChild($toast);
            document.getElementById('discard-autosave-button').addEventListener('click', function () {
        ...
        var autosaveRestored = false;
        if (!initialHash) {
            try {
                var rawAutosave = localStorage.getItem(AUTOSAVE_KEY);
                if (rawAutosave) {
                    var autosaveData = JSON.parse(rawAutosave);
                    if (hasAutosaveContent(autosaveData)) {
        ...
        // ---------- Warnung vor versehentlichem Schliessen ----------
        window.addEventListener('beforeunload', function (e) {
            var hasFrames = framesToPatterns().length > 0;
            var canvasHasContent = /[^f]/i.test(converter.fixPattern($hexInput.value));
            if (hasFrames || canvasHasContent) {
                e.preventDefault();
                e.returnValue = '';
            }
        });
